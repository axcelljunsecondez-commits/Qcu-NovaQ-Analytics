...
//#region node_modules/react-plotly.js/dist/chunk-WUFMRSJ6.mjs
...
function plotComponentFactory(Plotly) {
	return (0, import_react.forwardRef)(function PlotlyComponent({ data = DEFAULT_DATA, layout, config, frames, revision, onInitialized, onUpdate, onPurge, onError, debug = false, style = DEFAULT_STYLE, className, useResizeHandler = false, divId, ...eventProps }, forwardedRef) {
		const elRef = (0, import_react.useRef)(null);
		const promiseRef = (0, import_react.useRef)(Promise.resolve());
		const handlersRef = (0, import_react.useRef)({});
		const resizeHandlerRef = (0, import_react.useRef)(null);
		const unmountingRef = (0, import_react.useRef)(false);
		const prevRef = (0, import_react.useRef)(null);
		const onUpdateRef = (0, import_react.useRef)(onUpdate);
		onUpdateRef.current = onUpdate;
		const onPurgeRef = (0, import_react.useRef)(onPurge);
		onPurgeRef.current = onPurge;
		const handleUpdate = (0, import_react.useRef)(() => {
			const cb = onUpdateRef.current;
			if (typeof cb !== "function" || !elRef.current) return;
		...
		const setRef = (0, import_react.useCallback)((el) => {
			...
			if (debug && isBrowser && el) window.gd = el;
		}, [forwardedRef, debug]);
		(0, import_react.useEffect)(() => {
			...
			if (!figureChanged && (!revisionWasDefined || !revisionChanged)) return;
		...
		(0, import_react.useEffect)(() => {
			...
			return () => {
				...
				if (el) {
					...
					if (el.removeListener) updateEvents.forEach((evt) => el.removeListener(evt, handleUpdate));
					Plotly.purge(el);
				}
		...
		function runPlotlyReact(shouldInvokeResize, figureCallback, shouldAttachUpdateEvents) {
			promiseRef.current = promiseRef.current.then(() => {
				...
				if (!elRef.current) throw new Error("Missing element reference");
				return Plotly.react(elRef.current, {
					data,
					layout,
					config,
					frames
				...
				if (shouldAttachUpdateEvents) attachUpdateEvents();
			}).catch((err) => {
				if (typeof onError === "function") onError(err);
			});
		}
		function invokeFigureCallback(callback) {
			if (typeof callback !== "function" || !elRef.current) return;
			const el = elRef.current;
		...
		function attachUpdateEvents() {
			if (!elRef.current || !elRef.current.removeListener) return;
		...
		function syncWindowResize(invoke) {
			...
			if (useResizeHandler && !resizeHandlerRef.current) {
				...
				if (invoke) resizeHandlerRef.current();
		...
		function syncEventHandlers() {
			eventNames.forEach((eventName) => {
				...
				if (prop && !hasHandler) addEventHandler(eventName, prop);
				else if (!prop && hasHandler) removeEventHandler(eventName);
...
//#endregion
export { plotComponentFactory as default };
